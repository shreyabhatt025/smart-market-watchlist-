...
export function AppShell({ user, children }: { user?: User; children: ReactNode }) {
  ...
  return <div className="min-h-[100dvh] lg:grid lg:grid-cols-[248px_1fr]">
    ...
    <main className="min-w-0">
     <header className="flex min-h-[72px] items-center justify-between gap-4 border-b border-border/70 bg-background px-5 py-3 sm:px-8">
         <div className="flex min-w-0 items-center gap-3"><Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open menu" data-testid="button-open-menu"><Menu /></Button><span className="truncate text-sm font-semibold lg:hidden">Market Watchlist</span></div>
