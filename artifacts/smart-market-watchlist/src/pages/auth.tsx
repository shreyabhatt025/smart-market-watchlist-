...
export function AuthPage({ mode }: { mode: 'login' | 'register' }) {
  ...
  async function submit(event: FormEvent) {
   ...
   return <AuthLayout mode={mode}><div className="animate-rise"><p className="eyebrow text-primary">{isRegister ? 'Start your desk' : 'Welcome back'}</p><h2 className="mt-4 text-4xl font-bold leading-[1.08] tracking-[-.04em] sm:text-5xl">{isRegister ? 'A calmer way to keep watch.' : 'Return to your watch.'}</h2><p className="mt-5 text-sm leading-6 text-muted-foreground">{isRegister ? 'Set up a personal market read that respects your attention.' : 'Your latest attention read is waiting.'}</p>
