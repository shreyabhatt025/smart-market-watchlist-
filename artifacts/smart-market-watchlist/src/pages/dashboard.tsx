...
export function DashboardPage() {
  ...
  return <div className="mx-auto w-full max-w-[1400px] px-5 py-8 sm:px-8 sm:py-10 lg:px-12">
    <div className="animate-rise flex flex-col justify-between gap-6 border-b border-border/70 pb-8 sm:flex-row sm:items-end"><div><p className="eyebrow text-primary">Thursday · attention desk</p><h1 className="mt-3 font-display text-5xl leading-[.95] tracking-[-.04em] sm:text-6xl">What deserves<br /><em className="text-primary">your attention?</em></h1><p className="mt-5 max-w-lg text-sm leading-6 text-muted-foreground">A considered read of your watchlist, ranked by what changed since you last checked.</p></div><div className="flex flex-col items-start gap-3 sm:items-end"><MarketState state={market.data?.marketState} updatedAt={market.data?.updatedAt} stale={market.data?.items?.some((item) => item.isStale)} /><Button variant="outline" onClick={refresh} data-testid="button-refresh-market"><RefreshCw className="size-4" />Refresh read</Button></div></div>
    <div className="animate-rise animate-rise-1 mt-6 flex flex-col gap-3 sm:flex-row sm:items-center"><div className="relative max-w-lg flex-1"><Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-muted-foreground" /><Input value={query} onChange={(e) => { setQuery(e.target.value); setSearchOpen(true); }} onFocus={() => setSearchOpen(true)} placeholder="Add a stock by symbol or company" className="h-12 border-card-border bg-card pl-10" aria-label="Search stocks" data-testid="input-search-stocks" />{searchOpen && query.trim() && <div className="absolute inset-x-0 top-14 z-20 overflow-hidden rounded-xl border border-border bg-popover shadow-lg">{search.isLoading ? <div className="p-4 text-sm text-muted-foreground">Looking through listed companies…</div> : search.data?.length ? search.data.slice(0, 5).map((result) => <button key={result.symbol} type="button" onClick={() => addItem(result)} className="flex min-h-14 w-full items-center justify-between gap-3 px-4 text-left text-sm hover:bg-muted" data-testid={`button-add-${result.symbol}`}><span><span className="font-data text-primary">{result.symbol}</span><span className="ml-3 font-medium">{result.companyName}</span><span className="ml-2 text-xs text-muted-foreground">{result.exchange}</span></span><Plus className="size-4 text-primary" /></button>) : <div className="p-4 text-sm text-muted-foreground">No listed companies found for “{query}”.</div>}</div>}</div><Link href="/settings" className="focus-ring inline-flex min-h-12 items-center gap-2 rounded-lg border border-border bg-card px-4 text-sm font-medium hover:bg-muted" data-testid="link-settings-from-dashboard"><SlidersHorizontal className="size-4" />Tune thresholds</Link></div>
    ...
    {error ? <div className="mt-8"><QueryError onRetry={refresh} /></div> : isLoading ? <div className="mt-10"><LoadingRows count={3} /></div> : <div className="mt-10 space-y-12">
      ...
      <section className="rounded-xl border border-border/70 bg-primary px-5 py-6 text-primary-foreground sm:flex sm:items-center sm:justify-between sm:px-7"><div><p className="eyebrow text-primary-foreground/60">A note on the read</p><p className="mt-2 max-w-xl text-sm leading-6 text-primary-foreground/85">Attention is not a recommendation. It’s a gentle prompt to understand what changed, with your last check as the reference point.</p></div><span className="mt-5 block font-display text-3xl italic sm:mt-0">Stay curious.</span></section>
    </div>}
  </div>;
}