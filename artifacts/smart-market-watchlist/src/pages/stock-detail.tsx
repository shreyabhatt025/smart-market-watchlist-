...
export function StockDetailPage() {
  ...
  return <div className="mx-auto w-full max-w-[1240px] px-5 py-8 sm:px-8 sm:py-10 lg:px-12">
    ...
    <div className="mt-7 flex flex-col justify-between gap-6 border-b border-border/70 pb-8 md:flex-row md:items-end"><div><div className="flex items-center gap-3"><span className="font-data text-sm font-medium text-primary">{item.symbol}</span><span className="text-xs text-muted-foreground">{item.exchange} · {item.sector || 'Sector unavailable'}</span></div><h1 className="mt-3 font-display text-5xl tracking-[-.04em] sm:text-6xl">{item.companyName}</h1><div className="mt-4 flex flex-wrap items-center gap-3"><MarketState state={item.marketState} stale={item.isStale} /><span className="text-xs text-muted-foreground">Observed {formatDate(item.observedAt)}</span></div></div><div className="flex items-center gap-3"><Button variant="outline" onClick={markChecked} disabled={checkpoint.isPending} data-testid="button-update-checkpoint"><RotateCcw className="size-4" />{checkpoint.isPending ? 'Updating…' : 'Mark as checked'}</Button>{eventId && <Button onClick={acknowledgeSignal} disabled={acknowledge.isPending || acknowledged} data-testid="button-acknowledge-event">{acknowledged ? <Check className="size-4" /> : null}{acknowledged ? 'Acknowledged' : 'Acknowledge'}</Button>}</div></div>
